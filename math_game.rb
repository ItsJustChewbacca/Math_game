...
p1.name = p1.name.colorize(:cyan)
print 'Player 2 name: '
@username = gets.chomp
p2 = Player.new(@username)
p2.name = p2.name.colorize(:green)
...
while @repl_bool
  ...
  if @turn == 0 && question.is_correct?(@user_input)
    puts "CORRECT ANSWER!".colorize(:light_blue)
    p1.won
    @turn += 1
  ...
  puts "The score so far:
        #{p1.name}: #{p1.correct_answers} CORRECT ANSWERS #{p1.score} REMAINING LIVES
        #{p2.name}: #{p2.correct_answers} CORRECT ANSWERS #{p2.score} REMAINING LIVES"
  puts
